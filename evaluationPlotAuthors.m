function evaluationPlotAuthors(name, strategies, strategies_legend, specific_author_ids)
    ...
    if ~isdir(evaluation_dir)
       error('Evaluation files does not exists')
        ...
        for i = 1:length(authors_ids)
            if exist('specific_author_ids') && ~any(specific_author_ids==str2double(authors_ids{i}))
                continue
            end
